...
do 
    ...
    # ssh -n -i ~/.ssh/id_rsa -l ubc_eece411_5 ${node} "crontab -l"
    ...
    # scp -i ~/.ssh/id_rsa node.txt ubc_eece411_5@${node}:node.txt
    # scp -i ~/.ssh/id_rsa node.txt ubc_eece411_5@${node}:Group10/node.txt
    # scp -i ~/.ssh/id_rsa *.py ubc_eece411_5@${node}:Group10/.
    # scp -i ~/.ssh/id_rsa make-run.sh ubc_eece411_5@${node}:.
    # ssh -n -i ~/.ssh/id_rsa -l ubc_eece411_5 ${node} "echo '50 * * * * sh ~/make-run.sh >> ~/what.out 2>&1'  | crontab"
