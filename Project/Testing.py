...
def runTestCases(servers,file_object):
   ...
   # Invalid Command
    try:
        ...
        data, server = sendRequest(assembleMessage(5,"0","123"), server_address,file_object)
        if data:
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(1,"1","456"), server_address,file_object)
        if data:
          res_code,payload = parsePayload(data)
          assert res_code == 0
          data, server = sendRequest(assembleMessage(2,"1",""), server_address)
          res_code,payload = parsePayload(data)
          assert payload == "456"
          file_object.write("\nPassed\n\n")
          print "\nPassed\n"
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(1,"","/\\/\\/"), server_address,file_object)
        if data:
          res_code,payload = parsePayload(data)
          assert res_code == 0
          data, server = sendRequest(assembleMessage(2,""), server_address)
          res_code,payload = parsePayload(data)
          assert payload == "/\\/\\/"
          file_object.write("\nPassed\n\n")
          print "\nPassed\n"
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(1,"P.;",""), server_address,file_object)
        if data:
          res_code,payload = parsePayload(data)
          assert res_code == 0
          data, server = sendRequest(assembleMessage(2,"P.;"), server_address)
          res_code,payload = parsePayload(data)
          assert payload == ""
          file_object.write("\nPassed\n\n")
          print "\nPassed\n"
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(1,"1","123"), server_address)
        if data:
          res_code,payload = parsePayload(data)
          assert res_code == 0
...
#         data, server = sendRequest(assembleMessage(1,key,":lX"), server_address,file_object)
#         if data:
#           res_code,payload = parsePayload(data)
#           assert res_code == 0
#           data, server = sendRequest(assembleMessage(2,key), server_address)
#           res_code,payload = parsePayload(data)
#           assert payload == ":lX"
#           file_object.write("\nPassed\n\n")
#           print "\nPassed\n"
...
#         data, server = sendRequest(assembleMessage(1,'111',value), server_address,file_object)
#         if data:
#           res_code,payload = parsePayload(data)
#           assert res_code == 0
#           data, server = sendRequest(assembleMessage(2,'111'), server_address)
#           res_code,payload = parsePayload(data)
...
#         print "\nFailed\n"   
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(3,"4"), server_address)
        if data:
          sendRequest(assembleMessage(32,"4","ab&"), server_address,file_object)
          data, server = sendRequest(assembleMessage(2,"4"), server_address)
          res_code,payload = parsePayload(data)
          assert payload == "ab&"
          file_object.write("\nPassed\n\n")
          print "\nPassed\n"
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(3,""), server_address)
        if data:
          data, server = sendRequest(assembleMessage(32,"","..."), server_address,file_object)
          res_code,payload = parsePayload(data)
          assert res_code == 0
          data, server = sendRequest(assembleMessage(2,""), server_address)
          res_code,payload = parsePayload(data)
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(3,"0"), server_address)
        if data:
          data, server = sendRequest(assembleMessage(1,"0","4oop"), server_address)
          res_code,payload = parsePayload(data)
          assert res_code == 0
          data, server = sendRequest(assembleMessage(32,"0","xp*"), server_address,file_object)
          res_code,payload = parsePayload(data)
          assert res_code == 32
          file_object.write("\nPassed\n\n")
          print "\nPassed\n"
...
#             key += '3'
#         #print "Key to be sent: " + key
#         data, server = sendRequest(assembleMessage(3,key), server_address)
#         if data:
#           data, server = sendRequest(assembleMessage(1,key,"ppp"), server_address,file_object)
#           res_code,payload = parsePayload(data)
#           assert res_code == 0
#           data, server = sendRequest(assembleMessage(2,key), server_address)
#           res_code,payload = parsePayload(data)
...
#             value += '4'
#         #print "Value to be sent: " + value
#         data, server = sendRequest(assembleMessage(3,'\'M\fsdfdsfsdfdasfasff'), server_address)
#         if data:
#           data, server = sendRequest(assembleMessage(32,'\'M\fsdfdsfsdfdasfasff',value), server_address,file_object)
#           res_code,payload = parsePayload(data)
#           assert res_code == 0
#           data, server = sendRequest(assembleMessage(2,'\'M\fsdfdsfsdfdasfasff'), server_address)
#           res_code,payload = parsePayload(data)
#           assert payload == value
#           file_object.write("\nPassed\n\n")
#           print "\nPassed\n"
...
#         print "\nFailed\n"            
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(2,"f[pkpaksf\'"), server_address,file_object)
        if data:
          res_code,payload = parsePayload(data)
          assert res_code == 1
          file_object.write("\nPassed\n\n")
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(2,"f["), server_address)
        if data:
          res_code,payload = parsePayload(data)
          assert res_code == 1
          file_object.write("\nPassed\n\n")
    ...
    try:
        ...
        data, server = sendRequest(assembleMessage(3,"f++"), server_address,file_object)
        if data:
          res_code,payload = parsePayload(data)
          assert res_code == 1
          file_object.write("\nPassed\n\n")
...
#      
#              #print "Key to be sent: " + key
#              data, server = sendRequest(assembleMessage(1,"max",";;"), server_address)
#              if data:
#                sendRequest(assembleMessage(1,"max2",";;"), server_address)
#                data, server = sendRequest(assembleMessage(1,"max3",";;"), server_address,file_object)
#                res_code,payload = parsePayload(data)
#                assert res_code == 2
#                file_object.write("\nPassed\n\n")
#                print "\nPassed\n"
...
#              print "\nFailed\n"            
    ...
    try:
         ...
         data, server = sendRequest(assembleMessage(4), server_address)
         if data:
            assert False
         else:
             print "No response received."
             data, server = sendRequest(assembleMessage(1,"max2223",",,,"), server_address)
             if data:
               assert False
             else:
               file_object.write("No response received.\n")
    ...
    try:
        ...
        data, server =  sendRequest(assembleMessage(2,"input"), server_address)
        if data:
            res_code,payload = parsePayload(data)
    ...
    try:
         ...
         data, server = sendRequest(assembleMessage(2,"t1"), server_address2)
         if data:
           res_code,payload = parsePayload(data)
           assert payload == "good"
           data, server = sendRequest(assembleMessage(2,"t1"), server_address3)
           if data:
               res_code,payload = parsePayload(data)
               assert payload == "good"  
               file_object.write("\nPassed\n\n")
    ...
    try:
         ...
         data, server = sendRequest(assembleMessage(2,"a"), server_address3)
         if data:
           res_code,payload = parsePayload(data)
           assert payload == "123456"
           data, server = sendRequest(assembleMessage(2,"b"), server_address3)
           if data:
               res_code,payload = parsePayload(data)
               assert payload == "111111"  
               data, server = sendRequest(assembleMessage(2,"c"), server_address3)
               if data:
                   res_code,payload = parsePayload(data)
